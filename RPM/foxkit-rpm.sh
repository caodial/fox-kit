...
dnf_install_cmd=("sudo" "dnf" "install" "-y")
...
install_ide() {
    if command -v code &> /dev/null; then
        ...
        safe_execute sudo dnf check-update || true  # This command might return non-zero exit code when updates are available
        ...
        # Install VS Code using DNF (more appropriate than RPM for this case)
        safe_execute "${dnf_install_cmd[@]}" "$PACKAGE"
    fi
}
