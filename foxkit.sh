...
create_user() {
    ...
    mysql -u root -p -e "USE users;"
    mysql -u root -p -e "CREATE TABLE IF NOT EXISTS users (id INT AUTO_INCREMENT PRIMARY KEY, username VARCHAR(50), password VARCHAR(50));"
    mysql -u your_mysql_user -p -e "INSERT INTO users (username, password) VALUES ('$username', '$password');"
...
login_user() {
    ...
    echo
    result=$(mysql -u your_mysql_user -p -sse "SELECT COUNT(*) FROM users.users WHERE username='$username' AND password='$password';")
    if [ "$result" -eq 1 ]; then
...
# Add the create_user function to the menu
show_menu() {
